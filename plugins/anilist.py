...
from paimon.utils import media_to_image, check_owner, get_response
...
async def get_ani(vars_):
    ...
    for character in data["characters"]["nodes"]:
        ...
        html_ += (
            f"About Character and Role:{character.get('description', 'N/A')}"
        )
        html_char += f"{html_}"
    studios = "".join(" href='{}'>• {} ".format(studio["siteUrl"], studio["name"]) for studio in data["studios"]["nodes"])
    url = data.get("siteUrl")
